...
def run_produce_consume_cycle(produce_consumer_per_iteration=1):
    ...
    try:
        ...
        for i in range(produce_consumer_per_iteration):
            ...
            if i < produce_consumer_per_iteration - 1:  # Don't sleep after last iteration
                time.sleep(2)
        ...
        with open(log_filename, 'w') as log_file:
            log_file.write(f"Exception occurred at {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            log_file.write(f"{'=' * 80}\n")
...
def main():
    ...
    try:
        for i in range(args.iterations):
            ...
            success, log_file = run_produce_consume_cycle(produce_consumer_per_iteration=args.produce_consumer_per_iteration)
            if success:
                successful_runs += 1
            else:
                failed_runs += 1
